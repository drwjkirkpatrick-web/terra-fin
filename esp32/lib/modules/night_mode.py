"""Night mode sentinel — stationary monitoring during nighttime (ESP32/MicroPython).
...
class NightModeSentinel:
    ...
    Uses a simple running flag + _thread for the background poll loop.
    MicroPython has no threading.Event, so we use a boolean flag protected
    by a lock.
    """

    def __init__(self, config, sensors, event_bus, recorder):
        self._config = config
    ...
    def start(self):
        ...
        if hasattr(thread_mod, "start_new_thread"):
            thread_mod.start_new_thread(self._poll_loop, ())
        else:
            import threading
    ...
    def _poll_loop(self):
        ...
        while True:
            ...
            # Sleep for poll interval (using time.sleep, not Event.wait)
            import time
            time.sleep(getattr(self._config, "poll_interval_s", 30.0))
            ...
            if imu_reading is not None:
                ...
                if motion_delta > threshold:
                    ...
                    event = NightEvent(
                        event_type="motion",
                        timestamp=utc_now(),
                        description="Motion detected ({}, magnitude {:.2f})".format(
                            motion_class, motion_delta
                        ),
    ...
    @staticmethod
    def classify_motion(accel_magnitude_delta):
        ...
        Args:
            accel_magnitude_delta: |accel_magnitude - gravity| in m/s^2
